...
void BlockBrowser::updateExplorer(bool block)
{
    if(block)
    {
        ...
        if (pindex->IsProofOfStake()) {
            ...
            ui->hashRateBox->setText(QString::number(GetPoWMHashPS(pindex), 'f', 3) + " KH/s");
        }
        ui->moneySupplyBox->setText(QString::number(getMoneySupply(height), 'f', 6) + " GPL");
    }
    else {
        std::string txid = ui->txBox->text().toUtf8().constData();

        ui->valueBox->setText(QString::number(getTxTotalValue(txid), 'f', 6) + " GPL");
        ...
        ui->feesBox->setText(QString::number(getTxFees(txid), 'f', 6) + " GPL");
    }
}
